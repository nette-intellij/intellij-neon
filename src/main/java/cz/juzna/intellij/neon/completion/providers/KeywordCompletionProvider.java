...
//		if (params.getInvocationCount() >= 2)
		boolean incompleteKey = CompletionUtil.isIncompleteKey(curr);
		{
			...
			NeonKeyChain parent = CompletionUtil.getKeyChain(resolveKeyElementForChain(curr, incompleteKey));
		...
		if (parentKey instanceof NeonKey || curr instanceof NeonKey || curr.getParent() instanceof NeonKey || incompleteKey) { // key autocompletion
		...
		if (
				!(curr.getParent().getParent() instanceof NeonKey)
				&& !(curr.getParent() instanceof NeonClassUsage)
				&& !(curr.getParent() instanceof NeonKeyUsage)
				&& !(curr.getParent() instanceof NeonParameterUsage)
	...
	private boolean attachMainKeys(
		...
		if (keyValuePair == null || keyValuePair.getKey() == null) {
			parent = CompletionUtil.getKeyChain(keyValuePair != null ? keyValuePair : resolveKeyElementForChain(curr, incompleteKey));
		} else {
	...
	{
		if (isIncomplete || el.getParent() instanceof NeonFile) {
			return el.getParent();
		} else if (el.getParent().getParent() instanceof NeonFile) {
			return el.getParent();
		} else  {
			// literal -> value -> scalar -> [key ->] key-val pair -> any parent
			return el instanceof NeonKey ? el.getParent().getParent() : el.getParent().getParent().getParent();
		}
	}
