...
import cz.juzna.intellij.neon.NeonElementTypes;
...
public class NeonParserDefinition implements ParserDefinition {
    ...
    public PsiElement createElement(ASTNode node) {
	    ...
	    /*if(type == NeonTokenTypes.MACRO_NODE) return new MacroNodeImpl(node);
	    else if(type == NeonTokenTypes.MACRO_ATTR) return new MacroAttrImpl(node);
	    else*/ return new NeonPsiElement(node);
    }

    @Override
    public PsiFile createFile(FileViewProvider fileViewProvider) {
        return new NeonFileImpl(fileViewProvider);
    }
